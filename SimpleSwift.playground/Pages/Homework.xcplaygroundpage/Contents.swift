...
func calculate(_ args: [String]) -> Int {
    guard let last = args.last else { return -1 }
    ...
    let op = args[1]
    guard let firstNum = Int(args[0]),
          let secondNum = Int(args[2]) else {
        return -1
    }
    switch op {
    case "+":
...
func calculate(_ args: [String]) -> Double {
    guard let last = args.last else { return -1.0 }
    ...
    guard let firstNum = Double(args[0]),
          let secondNum = Double(args[2]) else {
        return -1.0
    }
...
func calculate(_ arg: String) -> Double {
    let rawParts = arg.split(separator: " ")
    var parts: [String] = []
    for word in rawParts {
